...
def create_model(pages, article, layouts, resorts, article_resorts, resort_page, layouts_pages, box_layouts, hull_layout_box, hull_article, article_length, hull_params, article_priority, alpha_value):
    g_under_fix = 0.5
    g_under_var = 0.49
    g_over_fix = 0.1
    g_over_var = 0.39
    ...
    M  = max(max_hull_value, max_article_length) +10
    M_prio = 1
    ...
    model.addConstrs(
        (f_page[j] ==
        quicksum(1/len(box_layouts[l]) * f_box[j,l,k] for l in layouts_pages[j] for k in box_layouts[l])for j in pages),
    ...
    #model.addConstr(y[1,6] == 1)
    #model.addConstr(x[2,1,10,3,28] == 1)
...
if __name__ == "__main__":
    ...
    instance_name = "mini_test_2_layouts"
    name = f"TBLOP mit Hüllen/Instances/{instance_name}.json"
    ...
    model = create_model(pages, article, layouts, resorts, article_resorts, resort_page, layouts_pages, box_layouts, hull_layout_box, hull_article, article_length, hull_params, article_priority, alpha_value=0.2)
