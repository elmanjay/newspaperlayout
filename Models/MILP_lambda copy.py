...
if __name__ == "__main__":
    ...
    if model.status == GRB.INFEASIBLE:
        ...
        model.write(os.path.join(instance_dir,"lp",f"{instance_name}.lp"))
        model.write(os.path.join(instance_dir,"sol", f"{instance_name}.sol"))
        solution_dict = {}
        ...
        for v in model.getVars():
            if abs(v.X) > 1e-9:    # numerisch sicher
                # z-Variablen zählen
